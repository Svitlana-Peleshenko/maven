...
public class CompareProductCharacteristicsInDifferentSectionsHelper {
	...
	public static void CompareProductInformation(Catalog catalog,
			...
			NavigateMenuProductInformation menu, Header compareLine) throws InterruptedException {

		Map<String, String> informationAboutProduct = new HashMap<>();
		int count = counts.intValue();
		int numElement = 1;
		Reporter.log("Check match the  product description in 'Catalog' and 'InformationAboutProduct' sections.");
	...
	public static void CheckMarkingRows(Catalog catalog,
			ComparativeTable table, Double counts) throws InterruptedException {
...
//		}
	//	catalog.getCompareButtons().get(count).click();
		Reporter.log("Check correct marking of rows in comparative table.");
